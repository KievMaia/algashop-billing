...
public class Invoice extends AbstractAuditableAggregateRoot<Invoice> {
    ...
    @ElementCollection
    @CollectionTable(name = "invoice_line_item", joinColumns = @JoinColumn(name = "invoice_id_"))
    ...
    public static Invoice issue(String orderId, UUID customerId, Payer payer, Set<LineItem> items) {
        ...
        if (StringUtils.isBlank(orderId)) throw new IllegalArgumentException("Order ID cannot be blank");
    ...
    public void markAsExpired() {
        setExpiredAt(OffsetDateTime.now().plusDays(3));
        setStatus(InvoiceStatus.CANCELED);
    }
    ...
    public void assignPaymentGatewayCode(String code) {
        if (!isUnpaid()) {
            throw new DomainException(String.format(
                    "Invoice %s with status %s cannot be edited", this.getId(), this.getStatus().toString().toLowerCase()));
        }
    ...
    public void changePaymentSettings(PaymentMethod method, UUID creditCardId) {
        ...
        var paymentSettings = PaymentSettings.brandNew(method, creditCardId);
